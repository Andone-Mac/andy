"""
FAL-SH: AI Summarizer API v2.5.1
...
CHINESE_LANDING_HTML = """<!DOCTYPE html>
<html lang=\"zh-CN\">
<head>
    <meta charset=\"UTF-8\">
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media (max-width: 600px) { .hero h1 { font-size: 2.5em; } .stats { gap: 30px; } .price-badge { font-size: 1.1em; padding: 12px 25px; } }
    </style>
</head>
<body>
    ...
    <div class=\"container\">
        ...
        <div class=\"demo-box\">
            <h3>🚀 在线体验 - 输入文本立即摘要</h3>
            <textarea class=\"demo-input\" id=\"demoText\" placeholder=\"在此输入要摘要的文本...\">人工智能正在改变企业的运营方式。从自动化日常任务到通过数据分析提供深度洞察，AI工具已成为各类公司必备的解决方案。中小企业现在可以通过经济实惠的API获取强大的AI能力，与大型竞争对手站在同一起跑线上。</textarea>
            <button class=\"demo-btn\" onclick=\"runDemo()\">🔍 智能摘要</button>
            <div class=\"demo-result\" id=\"demoResult\"></div>
        </div>
        <div class=\"features\">
            <div class=\"feature\"><h4>⚡ 4种摘要模式</h4><p>自动提取、要点列表、简短摘要、段落摘要。多种输出格式满足不同场景需求。</p></div>
            <div class=\"feature\"><h4>🌏 中文优先</h4><p>原生中文支持，自动检测语言，精准处理中文文本摘要。</p></div>
            <div class=\"feature\"><h4>📦 批量处理</h4><p>一次最多处理10条文本，批量处理享受8折优惠。</p></div>
            <div class=\"feature\"><h4>💳 简单计费</h4><p>按实际使用量计费，无月费无订阅。人民币定价，中国开发者友好。</p></div>
        </div>
        <div class=\"pricing\">
            <h2>透明定价 · 简单实惠</h2>
            <div class=\"pricing-grid\">
                <div class=\"price-card\"><div class=\"amount\">免费</div><div class=\"credits\">100 credits</div><div class=\"per\">无需信用卡</div><div class=\"cn\">新用户赠送</div></div>
                <div class=\"price-card\"><div class=\"amount\">¥70</div><div class=\"credits\">1,000 credits</div><div class=\"per\">¥0.07/千字符</div><div class=\"cn\">约处理100万字符</div></div>
                <div class=\"price-card\"><div class=\"amount\">¥700</div><div class=\"credits\">10,000 credits</div><div class=\"per\">¥0.07/千字符</div><div class=\"cn\">约处理1000万字符</div></div>
                <div class=\"price-card\"><div class=\"amount\">¥7000</div><div class=\"credits\">100,000 credits</div><div class=\"per\">¥0.07/千字符</div><div class=\"cn\">企业级套餐</div></div>
            </div>
        </div>
        <div class=\"payment-section\">
            <p style=\"color: #888; margin-bottom: 10px;\">支付方式</p>
            <div class=\"payment-icons\"><div class=\"payment-icon\">💳 信用卡</div><div class=\"payment-icon\">📱 支付宝</div><div class=\"payment-icon\">💬 微信支付</div><div class=\"payment-icon\">🎯 Stripe安全支付</div></div>
        </div>
        <div class=\"use-cases\">
            <h3>适用场景</h3>
            <div class=\"use-case-grid\">
                <div class=\"use-case\"><div class=\"icon\">📰</div><div class=\"title\">内容平台</div><div class=\"desc\">文章自动摘要</div></div>
                <div class=\"use-case\"><div class=\"icon\">📱</div><div class=\"title\">新闻聚合</div><div class=\"desc\">快速处理新闻</div></div>
                <div class=\"use-case\"><div class=\"icon\">📚</div><div class=\"title\">文档处理</div><div class=\"desc\">提取关键信息</div></div>
                <div class=\"use-case\"><div class=\"icon\">🤖</div><div class=\"title\">AI应用</div><div class=\"desc\">集成摘要能力</div></div>
                <div class=\"use-case\"><div class=\"icon\">📊</div><div class=\"title\">数据分析</div><div class=\"desc\">批量文本分析</div></div>
                <div class=\"use-case\"><div class=\"icon\">🎓</div><div class=\"title\">学术研究</div><div class=\"desc\">快速理解论文</div></div>
            </div>
        </div>
        <h2 style=\"margin: 40px 0 20px;\">快速开始</h2>
        <div class=\"code-example\">
            <span class=\"comment\"># 1. 获取免费API密钥</span><br>
            <span class=\"keyword\">curl</span> -X POST https://ai-summarizer-api-gswm.onrender.com/keys -H \"Content-Type: application/json\" -d '{\"name\":\"张三\",\"email\":\"zhangsan@example.com\"}'<br><br>
            <span class=\"comment\"># 2. 调用摘要接口</span><br>
            <span class=\"keyword\">curl</span> -X POST https://ai-summarizer-api-gswm.onrender.com/summarize -H \"Authorization: Bearer YOUR_KEY\" -H \"Content-Type: application/json\" -d '{\"text\":\"您的文本...\",\"max_length\":100,\"mode\":\"auto\"}'
        </div>
    ...
    <script>
        async function runDemo() {
            ...
            if (text.length < 30) { result.innerHTML = '<strong style=\"color:#f5576c;\">⚠️ 请输入至少30个字符</strong>'; result.classList.add('show'); return; }
            result.innerHTML = '<strong style=\"color:#667eea;\">⏳ 处理中...</strong>'; result.classList.add('show');
            try {
                const response = await fetch('/summarize', { method: 'POST', headers: { 'Authorization': 'Bearer fal_demo_abc123xyz789', 'Content-Type': 'application/json' }, body: JSON.stringify({ text: text, max_length: 100, mode: 'auto' }) });
                const data = await response.json();
                if (response.ok) { result.innerHTML = '<strong style=\"color:#4ade80;\">✅ 摘要结果：</strong><br><br>' + data.summary + '<br><br><small style=\"color:#888;\">📊 压缩率：' + Math.round(data.compression_ratio * 100) + '% | 消耗：' + data.credits_used + ' credit</small>'; }
                else { result.innerHTML = '<strong style=\"color:#f5576c;\">❌ 错误：</strong> ' + (data.detail || '未知错误'); }
            } catch (e) { result.innerHTML = '<strong style=\"color:#f5576c;\">❌ 网络错误：</strong> ' + e.message; }
        }
    </script>
</body>
</html>"""
...
async def chinese_landing():
    """Chinese landing page - 中文落地页"""
    return HTMLResponse(content=CHINESE_LANDING_HTML)
